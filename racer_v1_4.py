...
class NPV: #NPV - Non Player Vehicle
    ...
    def ahead(self, other):
        box_x = other.horizontal_position + other.car_width
        box_y = other.vertical_position
        box_h = other.car_height
        box_w = 250 #Distance to check for vehicles ahead
        return self.check_collision(box_x, box_y, box_w, box_h, self)
        #return (self.vertical_position == other.vertical_position and self.horizontal_position >= other.horizontal_position and (self.horizontal_position - other.horizontal_position) < 250)
...
class PlayerCrashHandler:
    ...
    def __init__(self, player):
        ...
        if (random.randrange(2) == 0) and player.car_vertical_position > RoadPositions.UPPER_LIMIT+20: 
            jolt = -20 
        elif player.car_vertical_position < RoadPositions.LOWER_LIMIT-20:
            jolt = 20
        else:
            jolt = -20
        ...
        self.timeout = 1700
        #self.timeout = (self.player.speed)*(Window.WIDTH + self.player.car_width) Why doesnt this work??!?
...
class Game(Gtk.Window):
    ...
    def load_images(self):
        ...
        self.available_cars = ( corvette, charger, golf, integra, supra, f430, ccx, db9, f1, superleggera, gt, lp570, murcielago, r8, rs4, sl65, slr)
        self.emergency_cars = ( ambulance, cop)
    ...
    def generateRandomNPV(self):
        ...
        if random_num > 4:
            return NPV(self.available_cars[random.randrange(len(self.available_cars))], lane, speed)
        else:
            return NPV(self.emergency_cars[random.randrange(len(self.emergency_cars))], lane, -20*Speed.ONE_KMH)
        ...
        for npv in self.npvs:
            if self.check_collision(self.car_horizontal_position, self.car_vertical_position, self.car_width, self.car_height, npv.horizontal_position, npv.vertical_position, npv.car_width, npv.car_height):
                ...
                if len(self.particleEmitters) < Particles.MAX_EMMITTERS:
                    ...
                    self.particleEmitters.append(PointsEmitter(self.particlePool, self.car_horizontal_position, self.car_vertical_position, -self.speed, 0.2))
                self.score -= 10
